/*
    Description: Read the microphone data of the PDM Unit and display the audio
   frequency spectrum. Note: Remove the M5GO base when using this example,
   otherwise it will not work properly,PDM Unit connected PORT-A
*/
...
//     M5.Lcd.drawString(string, 120, 3, 4);
// }
...
void microPhoneSetup()
{
    ...
    // spr.createSprite(240, 54);
    printf("microPhoneSetup done\n");
}
...
void MicroPhoneFFT()
{
    ...
    spr.fillRect(0, 0, 240, 54, spr.color565(0x00, 0x00, 0x00));
    uint32_t colorY = spr.color565(0xff, 0x9c, 0x00);
    uint32_t colorG = spr.color565(0x66, 0xff, 0x00);
    uint32_t colorRect;
    for (int x = 0; x < 24; x++)
    {
        for (int y = 0; y < 9; y++)
        {
            ...
            spr.fillRect(x * 10, 54 - y * 7 - 5, 5, 5, colorRect);
        }
    }
    // spr.pushSprite(0, 60);
