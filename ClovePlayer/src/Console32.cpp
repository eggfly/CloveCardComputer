...
Arduino_DataBus *bus = new Arduino_ESP32SPI(TFT_DC /* DC */, TFT_CS /* CS */, TFT_SCLK /* SCK */, TFT_MOSI /* MOSI */, GFX_NOT_DEFINED /* MISO */, FSPI /* spi_num */);

Arduino_GFX *screen = new Arduino_ST7789(
  bus, TFT_RST /* RST */, 1 /* rotation */, true /* IPS */,
  SCREEN_WIDTH /* width */, SCREEN_HEIGHT /* height */,
  0 /* col offset 1 */, 0 /* row offset 1 */);

Arduino_Canvas *gfx = new Arduino_Canvas(CANVAS_WIDTH /* width */, CANVAS_HEIGHT /* height */, screen);
...
void drawScreen() {
  ...
  {
    char batteryText[64];
    sprintf(batteryText, "%.2lfV", vbat);
    drawTextWithShadow(batteryText, 202, 7, RED, RGB565(0x80, 0x0, 0x0));
  }
...
void audio_process_extern(int16_t* buff, uint16_t len, bool *continueI2S) {
  ...
  *continueI2S = true;
  ...
  if (len >= 1024) {
    ...
    screen->draw16bitRGBBitmap(new_fft_pos + (CANVAS_WIDTH - 240) / 2, 22, (uint16_t*)new_fft_display_column_buff, 1, NEW_SPECTRUM_DISPLAY_HEIGHT);
  }
  printf("ch=%d, rate=%d, bits=%d, len=%d, cost %ld ms\n", channels, rate, bitsPerSample, len, millis() - start);
}
